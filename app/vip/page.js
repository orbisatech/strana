...
function VIPForm() {
  ...
  // Al cargar, verificar si la tarjeta ya está registrada
  useEffect(() => {
    const checkCard = async () => {
      try {
        ...
        // Si falla la consulta, mostrar formulario normal
      }
    }
    checkCard()
  }, [cardId])
  ...
  // Ya registrada — mostrar perfil
  if (status === 'registered' && profile) {
    return (
      <div style={{ minHeight: '100vh', background: '#050505', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        ...
        <p style={{ fontSize: '0.62rem', color: 'rgba(200,169,110,0.6)', letterSpacing: '0.15em', textTransform: 'uppercase', textAlign: 'center' }}>
          ✓ Active VIP Member
        </p>
        <p style={{ fontSize: '0.58rem', color: 'rgba(240,237,230,0.3)', marginTop: '0.5rem', textAlign: 'center', lineHeight: 1.8 }}>
          Present this card at the entrance to access your VIP benefits.
        </p>
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050505', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <style>{`
        .vip-input { width:100%; background:rgba(255,255,255,0.04); border:none; border-bottom:1px solid rgba(200,169,110,0.25); color:#fff; padding:1rem 0; font-family:'DM Sans',sans-serif; font-size:0.88rem; outline:none; transition:border-color 0.3s; box-sizing:border-box; letter-spacing:0.05em; }
        ...
        .vip-submit { width:100%; background:linear-gradient(135deg,#C8A96E 0%,#a8864e 100%); color:#050505; border:none; padding:1.1rem; font-family:'DM Sans',sans-serif; font-size:0.72rem; letter-spacing:0.22em; text-transform:uppercase; font-weight:700; transition:opacity 0.3s; cursor:pointer; margin-top:0.5rem; }
        .vip-submit:hover { opacity:0.88; }
        .vip-submit:disabled { opacity:0.4; cursor:not-allowed; }
        .field-hint { font-size:0.6rem; color:rgba(200,169,110,0.5); letter-spacing:0.08em; margin-top:0.4rem; }
      `}</style>

      <Image src="/logo.png" alt="STRANA" width={100} height={100} style={{ width: '75px', height: 'auto', marginBottom: '2.5rem' }} />

      {/* Black Card */}
      <div style={{ width:'100%', maxWidth:'380px', background:'linear-gradient(135deg,#1a1a1a 0%,#0d0d0d 50%,#1a1410 100%)', border:'1px solid rgba(200,169,110,0.2)', borderRadius:'16px', padding:'2rem', marginBottom:'2.5rem', boxShadow:'0 30px 80px rgba(0,0,0,0.8),inset 0 1px 0 rgba(200,169,110,0.15)', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:0, left:0, right:0, height:'1px', background:'linear-gradient(to right,transparent,rgba(200,169,110,0.4),transparent)' }} />
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'2rem' }}>
          <div>
            <p style={{ fontSize:'0.5rem', letterSpacing:'0.35em', textTransform:'uppercase', color:'rgba(200,169,110,0.6)', marginBottom:'0.3rem' }}>VIP Access</p>
      ...
      <div style={{ width:'100%', maxWidth:'380px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'1.5rem' }}>
          <div>
            <input className="vip-input" type="text" placeholder="Card Number" required value={form.cardNumber} onChange={e => setForm(f => ({ ...f, cardNumber: e.target.value }))} />
            <p className="field-hint">Enter the number printed on your VIP card</p>
          </div>
          <input className="vip-input" type="text" placeholder="Full Name" required value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} />
          <input className="vip-input" type="tel" placeholder="Phone Number (10 digits)" required pattern="[0-9]{10}" maxLength={10} value={form.telefono} onChange={e => setForm(f => ({ ...f, telefono: e.target.value }))} />
          <div>
            <input className="vip-input" type="date" required value={form.fecha} onChange={e => setForm(f => ({ ...f, fecha: e.target.value }))} style={{ colorScheme:'dark' }} />
        ...
        {status === 'error' && <p style={{ color:'#ff6b6b', fontSize:'0.72rem', marginTop:'1rem', textAlign:'center' }}>Something went wrong. Please try again or ask staff for help.</p>}
        <p style={{ fontSize:'0.55rem', color:'rgba(240,237,230,0.25)', marginTop:'2rem', textAlign:'center', lineHeight:1.9, letterSpacing:'0.05em' }}>Your information is confidential and will only be used to manage your VIP access at STRANA.</p>
      </div>
    </div>
  )
}
